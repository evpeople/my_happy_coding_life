...

	{
		...
		config = function()
			require("crates").setup({
				...
				select = {
					require_confirmation = true, -- 如果你想要选择后需要确认
					-- 在这里配置其他 select 选项
				},
			})
		end,
...
map("<leader>ct", crates.toggle, "[C]rates [T]oggle 显示")
...
map(
	...
	"将简单格式的 crate 声明扩展为内联表格格式。"
)
map("<leader>cX", crates.extract_crate_into_table, "将 crate 声明从任意格式提取到单独的表格格式。")
